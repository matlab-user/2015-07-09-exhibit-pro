...
---------------------------------------------------------------------------*/
	...
	function save_local_ip_port( $gid, $lip, $lport ) {
		...
		$sql_str = "UPDATE sp20150709_db.dev_t SET l_ip='".$lip."', l_port=".$lport." WHERE gid='".$gid."'";
	...
	function save_remote_ip_port( $gid, $rip, $rport ) {
		...
		$sql_str = "UPDATE sp20150709_db.dev_t SET d_ip='".$rip."', d_port=".$rport." WHERE gid='".$gid."'";
		mysql_query( $sql_str, $con );
	...
	// $t - 温度
	// 输入参数皆为字符串
	function save_data( $gid, $time, $t ) {
		global $mysql_user, $mysql_pass;
		$con = connect_mysql( $mysql_user, $mysql_pass );
		if( empty($con) )
			return;

		$sql_str = sprintf( "INSERT INTO sp20150709_db.data_t (dev_id,v_name,value,time,batch) VALUES ('%s','%s',%f,%d,%d)", $gid, 't', $t, $time, 0 );
		mysql_query( $sql_str, $con );
	...
	function get_remote_ip_port( $gid, &$r_ip, &$r_port ) {
		...
		$sql_str = sprintf( "SELECT d_ip, d_port FROM sp20150709_db.dev_t WHERE gid='%s'", $gid );
	...
	function save_N( $gid, $v_name, $th1, $th2 ) {
		...
		$sql_str = sprintf( "INSERT INTO sp20150709_db.normal_t SET gid='%s', v_name='%s', th1=%s, th2=%s", $gid, $v_name, $th1, $th2 );
		$res = mysql_query( $sql_str, $con );

		$sql_str = sprintf( "UPDATE sp20150709_db.normal_t SET th1=%s, th2=%s WHERE gid='%s' AND v_name='%s'", $th1, $th2, $gid, $v_name );
		$res = mysql_query( $sql_str, $con );
	...
	function get_normal( $gid ) {
		...
		$sql_str = sprintf( "SELECT th1, th2 FROM sp20150709_db.normal_t WHERE gid='%s' AND v_name='%s'", $gid, 'p' );
		$res = mysql_query( $sql_str, $con );
		...
		$sql_str = sprintf( "SELECT th1, th2 FROM sp20150709_db.normal_t WHERE gid='%s' AND v_name='%s'", $gid, 't' );
		$res = mysql_query( $sql_str, $con );
		if( $row = mysql_fetch_array($res) )
			$res_str .= $row[0].','.$row[1].',';
		else
		...
		$sql_str = sprintf( "SELECT th1, th2 FROM sp20150709_db.normal_t WHERE gid='%s' AND v_name='%s'", $gid, 'f' );
		$res = mysql_query( $sql_str, $con );
		if( $row = mysql_fetch_array($res) )
			$res_str .= $row[0].','.$row[1];
		else
